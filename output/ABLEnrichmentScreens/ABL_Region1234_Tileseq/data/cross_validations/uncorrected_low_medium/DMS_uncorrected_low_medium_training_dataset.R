...
ngr.df = read.csv("merged_table_imatinib_full_kinase_ngr_uncorr.csv",
...
for (i in 1:nrow(screen.df)) {
  ...
  netgrs = c(screen.df$netgr_obs_rep1.low[i],
             ...
             screen.df$netgr_obs_rep1.medium[i],
             screen.df$netgr_obs_rep2.medium[i],
...
write.csv(screen.df,"DMS_uncorrected_medium_training_set.csv")
