# 03.24.2023
# I run this script as a background job on R so that the variant caller doesn't occupy my R Session
# If there are any updates to the variant caller Rmd file (analysis/variant_caller_2022.Rmd), feel free to copy them over here
# Note that the inputs from the main Rmd file need to be initialized in the global environment first for this code to work
# Make sure you run the job in the main directory and include global environment variables to be fed into the code
library(stringr)
library(dplyr)
library(ggplot2)
library(plotly)
...
for(j in 1:length(seqdata_tsv_name)){
  # for(j in 1:1)){
  # for(j in 42:length(seqdata_tsv_name)){
  # j=1
