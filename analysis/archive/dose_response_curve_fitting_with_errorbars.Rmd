...
# write.csv(ic50data_all_sum,"ic50data_all_confidence_intervals_raw_data.csv")
...
# write.csv(ic50data_all_sum,"ic50data_all_confidence_intervals_individual_logistic_fits.csv")

```

One potential source of concern is that the upper confidence interval is not as far away from the mean as the lower interval. This happens because of converting %alive (y) to drug effect. The way I have done method 1 doesn't result in that problem but It's possible that method is erroneous in itself.
```{r}
ic50data_all_sum%>%filter(species=="M351T")%>%mutate(a=netgr_pred_sd_ul-netgr_pred_mean,b=netgr_pred_mean-netgr_pred_sd_ll)
```
