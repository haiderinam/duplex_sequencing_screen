...
ggplot(resmuts_merged%>%filter(!subs_name%in%"H396P"),aes(x=netgr_pred_mean,y=score,label=subs_name))+geom_text()+theme_bw()+xlab("IC50 Predicted Growth Rates")+ylab("Observed Enrichment Score")
# ggsave("BCRABL_EnrichmentCorrelation.pdf",width=4,height=4,units="in",useDingbats=F)

a=resmuts_merged%>%filter(!score%in%NA,!subs_name%in%"H396P")
...
sample1517=merge_samples("Novogene_lane16a/Sample8/sscs","Novogene_lane17/sample8/sscs")
sample1517=sample1517%>%
  rowwise()%>%
  mutate(ref_aa=strsplit(amino_acids,"/")[[1]][1],
         alt_aa=strsplit(amino_acids,"/")[[1]][2])
