...
```{r}
samplex=read.csv("data/Consensus_Data/Novogene_lane13/sample15/variant_caller_outputs/variants_unique_ann.csv",header=T,stringsAsFactors = F)
samplex=samplex%>%
  rowwise()%>%
...
a=il3D0%>%filter(consequence_terms%in%"missense_variant",protein_start>=242,protein_start<=494)
il3D0=merge_samples("Novogene_lane14/Sample10_combined","Novogene_lane13/Sample11")
il3D0=il3D0%>%
  rowwise()%>%
  mutate(ref_aa=strsplit(amino_acids,"/")[[1]][1],
         alt_aa=strsplit(amino_acids,"/")[[1]][2])
