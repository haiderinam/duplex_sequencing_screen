...
ggplot(netgr_corrected_errorbars%>%filter(!netgr_obs_sd%in%NA,!mutant%in%"E255K"),aes(x=factor(correction_status_name),y=netgr_obs_sd,group=mutant))+geom_point(color="black",size=3,shape=21,aes(fill=correction_status_name))+geom_line()+cleanup+
  theme(legend.position = "none",
        axis.text.y=element_blank(),
...
###Making boxplot: (ended up not looking good aesthetically)
netgr_corrected_compiled=netgr_corrected_compiled%>%
  mutate(correction_status_name=case_when(correction_status%in%"netgr_obs"~"Raw",
                                          correction_status%in%"netgr_obs_corrected"~"Corrected"))
netgr_corrected_compiled$correction_status_name=factor(netgr_corrected_compiled$correction_status_name,levels=c("Raw","Corrected"))

ggplot(netgr_corrected_compiled%>%filter(!netgr_obs%in%NA),aes(x=factor(correction_status_name),y=netgr_obs,fill=mutant))+
  geom_boxplot()+
...
maf_error_d6=twinstrand_merge_forplot_means%>%filter(count_type%in%"MAF",time_point%in%"D6")%>%ungroup()%>%dplyr::select(percent_error)
# raw=
