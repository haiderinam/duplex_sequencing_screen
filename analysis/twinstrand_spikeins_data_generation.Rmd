...
```
Looking at which samples I probably need to sequence again based on coverage
```{r}
#Spike in mutants
a=twinstrand_maf_merge%>%filter(CustomerName%in%c("M7D0","M7D3","M7D6"),tki_resistant_mutation=="True",!mutant%in%c("D276G"),mutant%in%c("F317L","F359V","F359I","E355G","E459K"))%>%dplyr::select(mutant,CustomerName,time_point,AltDepth,Depth)

plotly=ggplot(a,aes(x=time_point,y=AltDepth,color=factor(mutant),label=AltDepth))+geom_text(position = position_dodge(.5))+cleanup
ggplotly(plotly)

#ENU in mutants
a=twinstrand_maf_merge%>%filter(CustomerName%in%c("Enu4_D0","Enu4_D3","Enu4_D6"),!mutant%in%c("D276G"),mutant%in%c("H214R","F359V","K285N","A397P","E355G"))%>%dplyr::select(mutant,CustomerName,time_point,AltDepth,Depth)

plotly=ggplot(a,aes(x=time_point,y=AltDepth,color=factor(mutant),label=AltDepth))+geom_text(position = position_dodge(.5))+cleanup
ggplotly(plotly)

a=twinstrand_maf_merge%>%filter(CustomerName%in%c("Enu3_D3","Enu3_D6"),!mutant%in%c("D276G"),mutant%in%c("H214R","F359V","K285N","A397P","E355G"))%>%dplyr::select(mutant,CustomerName,time_point,AltDepth,Depth)
b=twinstrand_maf_merge%>%filter(CustomerName%in%"Enu3_D6")
c=twinstrand_maf_merge%>%filter(CustomerName%in%"Enu4_D0")
plotly=ggplot(a,aes(x=time_point,y=AltDepth,color=factor(mutant),label=AltDepth))+geom_text(position = position_dodge(.5))+cleanup
ggplotly(plotly)
sort(unique(twinstrand_maf_merge$CustomerName))
```
