...
# ggsave("enrichment_simulations_3mutant_MAF.pdf",width=2,height=2,units="in",useDingbats=F)
...
ggplot(sol_comb_doses%>%filter(time<=6,dose%in%1.2),aes(x=time,y=count,group=mutant))+
  ...
  theme(legend.position = "none",
        axis.title = element_text(face="bold",size=10),
        axis.text = element_text(face="plain",size=9))
# ggsave("enrichment_simulations_3mutant_count.pdf",width=2,height=2,units="in",useDingbats=F)

ggplot(sol_comb_doses%>%filter(dose%in%1.2)%>%filter(time%in%c(0,3)),aes(x=(factor(time)),y=proportion,fill=factor(mutant)))+geom_col(color="black")+facet_wrap(~mutant)+cleanup+discrete_scale("fill", "manual", palette_Dark2)+
  ...
  theme(strip.text = element_blank(),
        ...
        axis.title = element_text(face="bold",size=10),
        axis.text = element_text(face="plain",size=9))
# ggsave("allele_freq_enrichment_sim.pdf",width=3,height=2,units="in",useDingbats=F)

ggplot(sol_comb_doses%>%filter(dose%in%1.2)%>%filter(time%in%c(0,3)),aes(x=(factor(time)),y=count,fill=factor(mutant)))+geom_col(color="black")+facet_wrap(~mutant)+cleanup+discrete_scale("fill", "manual", palette_Dark2)+
...
# ggsave("count_enrichment_sim.pdf",width=3,height=2,units="in",useDingbats=F)
