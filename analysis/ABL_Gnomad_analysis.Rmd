...
imatdata_gnomad=imatdata%>%filter(gnomad_enabled%in%T)%>%dplyr::select(ref_aa,protein_start,alt_aa,netgr_obs_mean,species_enabler,gnomad_enabled,Allele.Frequency.Gnomad)
...
imatdata_gnomad_melt=melt(imatdata_gnomad,id.vars = c("species","species_enabler","ref_aa","protein_start","alt_aa","gnomad_enabled","Allele.Frequency.Gnomad","rank"),measure.vars =c("netgr.imat.low.1","netgr.imat.low.2","netgr.imat.mid.1","netgr.imat.mid.2","netgr.imat.high.1","netgr.imat.high.2"),
...
netgr_average=imatdata_gnomad%>%dplyr::select(species_enabler,species,netgr_average,rank)
...
ggplot(imatdata_gnomad%>%arrange(desc(rank)),aes(x=reorder(species, -netgr_average),y=netgr_average,label=species,size=Allele.Frequency.Gnomad))+
  ...
  geom_text(data = netgr_average, aes(x = species, y = netgr_average, label = species),
            position = position_nudge(y = 0.04), # Adjust the position if needed
            size = 2, color = "red",
            angle=90)+
    geom_text(aes(label = "\u2191"), size = 4,position = position_nudge(y = 0.03)) +  # Add arrow shape
  geom_text(data = netgr_average, aes(x = species, y = netgr_average, label = species_enabler),
            position = position_nudge(y = 0.02), # Adjust the position if needed
            size = 2, color = "blue",
            angle=90)+
  scale_x_discrete("SNP-enabled ABL mutant")+
...
ggplot(imatdata_gnomad%>%arrange(desc(rank)),aes(x=reorder(species_name, -netgr_average),y=netgr_average,label=species,size=Allele.Frequency.Gnomad))+
  geom_point(color="black",fill="gray90",shape=21)+
  scale_x_discrete("SNP-enabled ABL mutant")+
  scale_y_continuous("Net growth rate (Hrs ^-1)")+
  cleanup+
...
Making a few piecharts for the paper
```{r}
library(tools)
...
# ggsave("output/ABLEnrichmentScreens/ABL_Region234_Comparisons/sm_imatinib_plots/gnomad_piecharts.pdf",width=3,height = 3, units="in",useDingbats=F)
source("code/resmuts_adder.R")
imatdata=resmuts_adder(imatdata)
ggplot(imatdata%>%filter(!species%in%"V299L"),aes(y=netgr_obs_mean,fill=resmuts))+
