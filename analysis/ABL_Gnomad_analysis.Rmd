...
#Aim is to create a list of gnomad enabled mutants, i.e. mutants that would have required multi-nucleotide variants before, but are now a single codon away
...
gnomad_simple=gnomad_simple%>%
  ...
  mutate(ref_codon=(ref_genomic_coordinates[(position-ref_genomic_coordinates$start)%in%c(0,1,2),"codon"])[[1]],
         ...
         alt_codon=case_when(alt_offset%in%1~paste(Alternate,substr(ref_codon,2,3),sep=""),
                             alt_offset%in%3~paste(substr(ref_codon,1,2),Alternate,sep = ""),
...
# a=gnomad_enabled%>%filter(protein_start%in%c(242:494))
...
sd(imatdata$netgr_obs_mean)
0.003016086+0.03076209+0.03076209
quantile(imatdata$netgr_obs_mean,c(.5,.75,.85))
a=gnomad%>%filter(residue%in%473)
a=gnomad%>%filter(residue%in%247)
# a=gnomad_simple%>%filter(residue%in%473)
...
a=clinvar%>%filter(residue%in%c("473","247"))
