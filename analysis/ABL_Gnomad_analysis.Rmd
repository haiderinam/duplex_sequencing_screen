...
# Sample ethnicity distribution for each point
ethnicity_data <- list(
  data.frame(ethnicity = c("A", "B", "C"), value = c(50, 30, 20)),
  data.frame(ethnicity = c("A", "B", "C"), value = c(40, 40, 20)),
  data.frame(ethnicity = c("A", "B", "C"), value = c(60, 20, 20)),
  data.frame(ethnicity = c("A", "B", "C"), value = c(30, 50, 20)),
  data.frame(ethnicity = c("A", "B", "C"), value = c(20, 50, 30))
)

# Function to create pie chart grob
make_pie_grob <- function(data) {
  p <- ggplot(data, aes(x = "", y = value, fill = ethnicity)) +
...
p <- ggplot(data, aes(x, y)) +
  geom_blank() +  # Keeps the base plot structure
  theme_minimal()

# Add pie charts at each point
for (i in 1:nrow(data)) {
  pie_grob <- make_pie_grob(ethnicity_data[[i]])
  p <- p + annotation_custom(pie_grob, xmin = data$x[i] - 0.4, xmax = data$x[i] + 0.4, 
                             ymin = data$y[i] - 0.05, ymax = data$y[i] + 0.05)
}
...
resistance_predictions=resistance_predictions%>%dplyr::select(species,ref_aa,protein_start,alt_aa,fill_value,alpha.444)
...
imatdata_gnomad=imatdata%>%filter(gnomad_enabled%in%T)%>%dplyr::select(dose,species,ref_aa,protein_start,alt_aa,netgr_obs_mean,netgr_obs_mean_corr,species_enabler,gnomad_enabled,Allele.Frequency.Gnomad,fill_value,alpha.444)
