...
ggplot(a%>%filter(!alt_aa%in%"O",protein_start>320,protein_start<390),aes(x=protein_start,y=alt_aa,fill=ct))+geom_tile()
...
ggplot(b%>%filter(!alt_aa%in%"O",protein_start>390,protein_start<450),aes(x=protein_start,y=alt_aa,fill=ct))+geom_tile()


# Region 4:
b=merge_samples("novogene_lane23/Lane23_Sample19/sscs","Novogene_lane19_20/Ln19b_Sample9/sscs")
ggplot(b%>%filter(!alt_aa%in%"O",protein_start>450,protein_start<520),aes(x=protein_start,y=alt_aa,fill=ct))+geom_tile()
```
...
heatmap_plotting_function(screen_compare_means,470,520,fill_variable = "netgr_obs_mean",fill_name = "Net growth rate")
