...
ggplot(data%>%filter(protein_start>=242,protein_start<=512),aes(x=netgr_obs_mean,fill=resmuts))+
...
################Plot 2b#################
ggplot(data%>%filter(protein_start>=242,protein_start<=512),aes(x=netgr_obs_mean_corr,fill=resmuts))+
  geom_density(alpha=.7)+
  scale_x_continuous(bquote('Net Growth Rate '(Hours^-1)),limits=c(-.03,.09))+
  scale_y_continuous("Density")+
  scale_fill_manual(values=c("gray","red"))+
