...
ggsave(paste(dirname,"1a.baf3_imatinib_heatmap_netgr_",dose,"_",date,".pdf",sep=""),width=16,height=4,units="in",useDingbats=F)
...
ggsave(paste(dirname,"1b.baf3_imatinib_heatmap_netgr_corrected_",dose,"_",date,".pdf",sep=""),width=16,height=4,units="in",useDingbats=F)

################Plot 2a#################
#   a=data%>%filter(is_intended%in%1)
ggplot(data%>%filter(protein_start>=242,protein_start<=512),aes(x=netgrowth_mean,fill=resmuts))+
...
ggsave(paste(dirname,"2a.baf3_imatinib_histograms_netgr_",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)
################Plot 2b#################
ggplot(data%>%filter(protein_start>=242,protein_start<=512),aes(x=netgrowth_mean_corr,fill=resmuts))+
  geom_density(alpha=.7)+
  scale_x_continuous(bquote('Net Growth Rate '(Hours^-1)),limits=c(-.03,.09))+
  scale_y_continuous("Density")+
  scale_fill_manual(values=c("gray","red"))+
...
ggsave(paste(dirname,"2b.baf3_imatinib_histograms_netgr_corrected_",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)
...
ggsave(paste(dirname,"3a.baf3_imatinib_correlations_netgr_",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)
...
ggsave(paste(dirname,"3b.baf3_imatinib_correlations_netgr_corrected_",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)
...
ggplot(standards,aes(x=.055-drug_effect,y=netgrowth_mean,label=species))+
  ...
  geom_abline(linetype="dashed")+
  cleanup
ggsave(paste(dirname,"4a.baf3_imatinib_mutantstandards_netgr_",dose,"_",date,".pdf",sep=""),width=3,height = 3,units="in",useDingbats=F)
...
ggplot(standards,aes(x=.055-drug_effect,y=netgrowth_mean_corr,label=species))+
  ...
  geom_abline(linetype="dashed")+
  cleanup
ggsave(paste(dirname,"4b.baf3_imatinib_mutantstandards_netgr_corrected_",dose,"_",date,".pdf",sep=""),width=3,height = 3,units="in",useDingbats=F)
...
ggsave(paste(dirname,"5a.baf3_imatinib_sangermutants_netgr_",dose,"_",date,".pdf",sep=""),width=2.5,height=3,units="in",useDingbats=F)
...
ggsave(paste(dirname,"5b.baf3_imatinib_sangermutants_netgr_corrected_",dose,"_",date,".pdf",sep=""),width=2.5,height=3,units="in",useDingbats=F)
...
ggsave(paste(dirname,"6a.baf3_imatinib_pre-existingALL_netgr_",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)
...
ggplot(data_merged%>%filter(!species%in%"V299L",n_nuc_min%in%1),aes(x=resmut_cosmic,y=netgrowth_mean_corr,fill=resmut_cosmic))+
  ...
  geom_jitter(color="black", size=.5,width=.1, alpha=0.9)+
  scale_fill_manual(values=c("gray90","gray50","red"))+scale_y_continuous(name=bquote('Net Growth Rate '(Hours^-1)))+scale_x_discrete("",labels=c("Undetected","Pre-existing\n ALL","Known\nResistant\n Variant"))+cleanup+theme(legend.position = "none")
ggsave(paste(dirname,"6b.baf3_imatinib_pre-existingALL_netgr_corrected_",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)
...
ggplot(t315_merge,aes(x=lfc,y=netgrowth_mean,label=species))+geom_text(size=2.5)+scale_x_continuous(name=ylabel)+scale_y_continuous(bquote('Net Growth Rate '(Hours^-1)))+stat_cor(method="pearson")+cleanup
ggsave(paste(dirname,"7a.baf3_imatinib_T315xdataset_netgr_",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)

################Plot 7b#################
ggplot(t315_merge,aes(x=lfc,y=netgrowth_mean_corr,label=species))+geom_text(size=2.5)+scale_x_continuous(name=ylabel)+scale_y_continuous(bquote('Net Growth Rate '(Hours^-1)))+stat_cor(method="pearson")+cleanup
ggsave(paste(dirname,"7b.baf3_imatinib_T315xdataset_netgr_corrected",dose,"_",date,".pdf",sep=""),width=4,height=4,units="in",useDingbats=F)
...
```{r}
data=read.csv("output/ABLEnrichmentScreens/ABL_Region1234_Tileseq/data/TileSeq_full_kinase_low_imatinib_final_renamed.csv",header = T,stringsAsFactors = F)
ggplot(data,aes(x=factor(protein_start),y=ct_screen1_before,fill=factor(is_intended)))+geom_boxplot()+cleanup+scale_y_continuous(trans="log10")+theme(axis.text.x=element_text(angle=90, hjust=1))

ggsave(paste(dirname,"coverage_across_ABL",dose,"_",date,".pdf",sep=""),width=36,height=4,units="in",useDingbats=F)
